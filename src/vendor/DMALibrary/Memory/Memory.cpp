...
bool Memory::SetFPGA()
{
	...
	if ((qwVersionMajor >= 4) && ((qwVersionMajor >= 5) || (qwVersionMinor >= 7)))
	{
		HANDLE handle;
			   ...
			   strcpy(config.szDevice, "existing");
...
bool Memory::GetHeapRegions(std::vector<HeapRegion>& heaps) {
    PVMMDLL_MAP_VAD pVadMap = nullptr;
    heaps.clear();

    // use this->vHandle and current_process.PID directly
    bool ok = VMMDLL_Map_GetVadU(this->vHandle, current_process.PID, FALSE, &pVadMap);
    if (!ok || !pVadMap) {
        return false;
    }
    ...
    for (DWORD i = 0; i < pVadMap->cMap; ++i) {
        ...
        if (vad.fPrivateMemory == 1 &&
            ...
            vad.VadType == 0) {
            heaps.push_back({
                static_cast<uintptr_t>(vad.vaStart),
                static_cast<uintptr_t>(vad.vaEnd)
            });
        }
    }
